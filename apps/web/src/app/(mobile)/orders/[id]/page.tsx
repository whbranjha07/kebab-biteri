...
export default function OrderDetailPage() {
  ...
  const { order, loading, error, attempts } = useOrder(params.id)
  ...
  // Definitive errors we shouldn't keep spinning on — the poll won't recover.
  const definitiveStatus = error instanceof ApiError && [401, 403, 404].includes(error.statusCode) ? error.statusCode : null

  // Keep showing a friendly "syncing" state during the first few polls —
  // a fresh order can 404 briefly while the DB write settles / API cold-starts.
  const stillTrying = !order && !definitiveStatus && attempts < 4

  if (loading || stillTrying) {
    return (
      <div className="flex min-h-dvh flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
          ...
          <p className="text-sm text-muted">
            {attempts === 0 ? 'Loading your order…' : 'Just a moment — syncing your order…'}
          </p>
        </div>
      </div>
    )
  ...
  if (!order) {
    let title = "We couldn't reach your order"
    let hint = "Your order was placed. This page is having trouble syncing — reload, or check your orders."
    let showLogin = false

    if (definitiveStatus === 401) {
      title = 'Please log in to view this order'
      hint = 'Your session may have expired. Log in again to see the details.'
      showLogin = true
    } else if (definitiveStatus === 403) {
      title = 'This order belongs to a different account'
      hint = 'Log in with the account you used to place the order, or return to your orders list.'
      showLogin = true
    } else if (definitiveStatus === 404) {
      title = 'Order not found'
      hint = "We couldn't find this order. It may have been placed as a guest — check your email or SMS for the receipt link."
    }

    return (
      <div className="flex min-h-dvh flex-col items-center justify-center px-6 text-center">
        <AlertCircle className="h-12 w-12 text-danger" />
        <p className="mt-4 text-lg font-semibold text-charcoal">{title}</p>
        <p className="mt-1 max-w-sm text-sm text-muted">{hint}</p>
        <div className="mt-5 flex gap-3">
          {showLogin ? (
            <Link href="/profile/login"><Button>Log in</Button></Link>
          ) : (
