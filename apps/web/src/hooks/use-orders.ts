...
export function useOrder(orderId: string) {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setOrder(res)
          setError(null) // clear any prior transient error
          setLoading(false)
        }
      } catch (e) {
        if (!cancelled) {
          setAttempts((n) => n + 1)
          // Only surface the error if we don't already have order data.
          setError((prev) => (order ? prev : (e as Error)))
          setLoading(false)
          // If it's a definitive "won't ever succeed" error (auth/forbidden/not-found),
          // stop polling so we don't hammer the API.
          if (e instanceof ApiError && [401, 403, 404].includes(e.statusCode)) {
  ...
  return { order, loading, error, attempts }
}
